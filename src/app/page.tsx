import { homeMetadata, createJsonLd } from '@/lib/seo'
import type { PostListItem } from '@/services/notion/client'
import { listPublishedPostsMemo, getSiteSettingsMemo } from '@/lib/request-memo'
...
export default async function Home() {
  ...
  try {
    ...
    // 에러 시에도 설정은 가져오기 시도
    try {
      settings = await getSiteSettingsMemo()
    } catch {
      settings = {
        homeTitle: '블로그 제목',
  ...
  const websiteJsonLd = createJsonLd({
    type: 'WebSite',
    title: settings?.homeTitle || '블로그 제목',
    description: settings?.homeSubtitle || 'Notion으로 관리하는 개인 블로그',
    url: SITE_URL,
  })
  ...
  const blogJsonLd = createJsonLd({
    ...
    description: settings?.homeSubtitle || 'Notion으로 관리하는 개인 블로그',
    url: SITE_URL,
    author: settings?.authorName || '작성자 이름',
  })
