...
3. [포스팅 데이터베이스 설정](#2-포스팅-데이터베이스-설정)
   ...
   - [2.6 데이터베이스 ID 가져오기](#26-데이터베이스-id-가져오기)
     ...
     - [단계 3: 하이픈 제거하기](#단계-3-하이픈-제거하기-있는-경우)
     - [✅ 확인하기](#-확인하기)
   ...
   - [2.9 소셜 미디어 공유 최적화](#29-소셜-미디어-공유-최적화)
     ...
     - [CoverImage 설정 방법](#coverimage-설정-방법-필수)
     - [소셜 미디어별 이미지 최적화](#소셜-미디어별-이미지-최적화)
     - [공유 미리보기 테스트 방법](#공유-미리보기-테스트-방법)

4. [프로필 설정 데이터베이스 구성](#3-프로필-설정-데이터베이스-구성)
...
5. [사이트 설정 데이터베이스 구성](#4-사이트-설정-데이터베이스-구성)
...
6. [About 페이지 구성](#5-about-페이지-구성)
   - [5.1 About 페이지 생성](#51-about-페이지-생성)
   - [5.2 페이지 ID 가져오기](#52-페이지-id-가져오기)
   - [5.3 Integration 연결](#53-integration-연결)

7. [환경 변수 설정](#6-환경-변수-설정)
   - [🔐 환경 변수 설정 플로우](#-환경-변수-설정-플로우)
   - [6.1 `.env.local` 파일 생성하기](#61-envlocal-파일-생성하기)
   - [6.2 환경 변수 설명](#62-환경-변수-설명)
   - [6.3 GitHub Actions 설정](#63-github-actions-설정-배포용---필수)
...
- 다시 "Publish"로 변경하면 다음 빌드 시(최대 10분 내) 목록에 다시 표시됩니다
...
**해결:**
1. 블로그가 최근에 재빌드되었는지 확인 (최대 10분 대기)
...
| **HomeDescription** | Text | ⬜ | 홈페이지 설명 | 개발, 디자인, 일상에 대한 생각을 공유합니다. |
...
- ✅ **10분마다** Notion 데이터를 GitHub Pages에 빌드
...
#### 콘텐츠 업데이트
- Notion에서 수정하면 최대 10분 내에 블로그에 반영됨
